...
const getBids = async (bidders, conf, debug) => Promise.all(
	bidders.map(async bidder => {
		...
		const { generate_bid } = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(bidder.bidding_logic_url)); });
...
const getScores = async (bids, conf, debug) => {
	const { score_ad } = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(conf.decision_logic_url)); });
...
const getSellerReport = async (conf, results) => {
	const { report_result } = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(conf.decision_logic_url)); });
...
const getBuyerReport = async (conf, results, report) => {
	const { report_win } = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(results.bid.bidding_logic_url)); });
